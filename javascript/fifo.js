function FIFOfunc(cantidad, llegada, duracion ){
    var procesos= [];
    var tiempototal = 0;
    ...
    for (let i = 0; i < cantidad; i++) {
      for (let j = 0; j < procesos[i].duracion; j++) {
        cola[procesos[i].tiempoinicio+j]="P"+String(i+1);
      }
    }
    console.log(cola);
    ...
    tr.appendChild(td)
    var td = document.createElement('td');
    td.appendChild(document.createTextNode('tiempo de inicio'));
    tr.appendChild(td);
    tbdy.appendChild(tr);
    for (let j = 0; j < cantidad; j++) {
      var tr = document.createElement('tr');
        ...
        var td = document.createElement('td');
        td.appendChild(document.createTextNode(procesos[j].tiempoinicio));
        tr.appendChild(td);
      tbdy.appendChild(tr);
    }
